...
public class Generator {
    ...
    public void generateDepartments(int depCount) {
            ...
            for (int i = 0; i < depCount; i++) {		
            	MainClass.setDepartment(i, new Department(names[i], types[i], i)); // выбор имени и типа подразделения по порядку
    ...
    public void generateClasses(int classCount, int depCount, int maxArea) {
            ...
            for (int i = 0; i < classCount; i++) {
                ...
                MainClass.getDepartment(departmentIndex).getClasses().add(num); // добавление помещения в список помещений подразделения
